...
// 辅助函数：根据关节类型和相对索引获取压缩描述符
static std::string GetStateDescriptor(int joint_type, int relative_index, bool is_qvel) {
  switch (joint_type) {
    case mjJNT_FREE:
      if (is_qvel) {
    ...
    case mjJNT_BALL:
      ...
      break;
    case mjJNT_SLIDE:
      return is_qvel ? "vel" : "pos";
    case mjJNT_HINGE:
      return is_qvel ? "av" : "ang";  // 压缩：avel → av, angle → ang
  }
  return "unk"; // Should not happen for valid joints
}

// 辅助函数：根据关节类型获取 qpos 数量
static int GetQposNumForJoint(int joint_type) {
  switch(joint_type) {
...
// 辅助函数：根据关节类型获取 qvel (dof) 数量
static int GetQvelNumForJoint(int joint_type) {
  switch(joint_type) {
...
// 创建 State (qpos/qvel) UI Section
static void MakeStateSection(mj::Simulate* sim, const mjModel* m) {
  if (!m) return;

  // 清理旧的 state_item_infos_ 和重置 ID
  sim->state_item_infos_.clear();
  sim->state_sect_id_ = -1;

  // 定义主 Section
  mjuiDef defStateSection[] = {
    {mjITEM_SECTION, "State (qpos/qvel)", mjPRESERVE, nullptr, "AS"},
    {mjITEM_END}
  };

  // 添加主 Section
  mjui_add(&sim->ui1, defStateSection);
  sim->state_sect_id_ = sim->ui1.nsect - 1;

  // 定义模板
  mjuiDef defSeparator[] = {
    {mjITEM_SEPARATOR, "", 1},
    {mjITEM_END}
  };

  mjuiDef defStaticItem[] = {
  ...
  for (int jnt_id = 0; jnt_id < m->njnt && total_items_added < mjMAXUIITEM - 30; jnt_id++) {
    int joint_type = m->jnt_type[jnt_id];

    // 获取关节名称
    std::string joint_name;
    if (sim->jnt_names_[jnt_id].empty()) {
      joint_name = "joint_" + std::to_string(jnt_id);
    } else {
      joint_name = sim->jnt_names_[jnt_id];
    }

    // 添加 qpos 分隔符
    std::string qpos_section_name = joint_name + "_qpos";
    mju::strcpy_arr(defSeparator[0].name, qpos_section_name.c_str());
    mjui_add(&sim->ui1, defSeparator);
    total_items_added++;

    // 添加 qpos 元素
    int qpos_start = m->jnt_qposadr[jnt_id];
    int qpos_num = GetQposNumForJoint(joint_type);
    for (int i = 0; i < qpos_num; i++) {
      int global_qpos_idx = qpos_start + i;
      std::string descriptor = GetStateDescriptor(joint_type, i, false);

      // 创建简化显示文本：索引:描述符,数值
      char display_text[mjMAXUINAME];
      snprintf(display_text, mjMAXUINAME, "%d:%s,0.000",
               global_qpos_idx, descriptor.c_str());
      display_text[mjMAXUINAME - 1] = '\0';

      mju::strcpy_arr(defStaticItem[0].name, display_text);
      mjui_add(&sim->ui1, defStaticItem);
      total_items_added++;

      // 记录状态项信息 - 使用当前 Section 的 item 计数
      mj::Simulate::StateItemInfo info;
      info.joint_id = jnt_id;
      info.q_index = global_qpos_idx;
      info.is_qvel = false;
      info.descriptor = descriptor;
      info.ui_item_id = sim->ui1.sect[sim->state_sect_id_].nitem - 1;
      sim->state_item_infos_.push_back(info);
    }

    // 添加 qvel 分隔符
    std::string qvel_section_name = joint_name + "_qvel";
    mju::strcpy_arr(defSeparator[0].name, qvel_section_name.c_str());
    mjui_add(&sim->ui1, defSeparator);
    total_items_added++;

    // 添加 qvel 元素
    int qvel_start = m->jnt_dofadr[jnt_id];
    int qvel_num = GetQvelNumForJoint(joint_type);
    for (int i = 0; i < qvel_num; i++) {
      int global_qvel_idx = qvel_start + i;
      std::string descriptor = GetStateDescriptor(joint_type, i, true);

      // 创建简化显示文本：索引:描述符,数值
      char display_text[mjMAXUINAME];
      snprintf(display_text, mjMAXUINAME, "%d:%s,0.000",
               global_qvel_idx, descriptor.c_str());
      display_text[mjMAXUINAME - 1] = '\0';
...
// 更新状态监视显示的实时数值
static void UpdateStateSection(mj::Simulate* sim) {
  if (!sim->m_ || !sim->d_ || sim->state_sect_id_ < 0) return;

  // 检查 State Section 是否展开
  if (!sim->ui1_enable || !sim->ui1.sect[sim->state_sect_id_].state) {
    return; // Section 未展开，跳过更新以提高性能
  }

  // 遍历所有状态项，更新显示文本
  for (const auto& info : sim->state_item_infos_) {
    if (info.ui_item_id >= sim->ui1.sect[sim->state_sect_id_].nitem) continue; // 安全检查

    mjuiItem* item = &sim->ui1.sect[sim->state_sect_id_].item[info.ui_item_id];

    // 获取当前数值
    mjtNum current_value;
    if (info.is_qvel) {
      if (info.q_index < sim->m_->nv) {
        current_value = sim->d_->qvel[info.q_index];
      } else {
        current_value = 0.0; // 安全默认值
      }
    } else { // qpos
      if (info.q_index < sim->m_->nq) {
        current_value = sim->d_->qpos[info.q_index];
      } else {
        current_value = 0.0; // 安全默认值
      }
    }

    // 格式化新的简化显示文本：索引:描述符,数值
    char new_full_name[mjMAXUINAME];

    // 使用最简化的格式
    snprintf(new_full_name, mjMAXUINAME, "%d:%s,%.3f",
             info.q_index,
             info.descriptor.c_str(),
             (double)current_value);

    new_full_name[mjMAXUINAME - 1] = '\0';

    // 更新 item 名称
    mju::strcpy_arr(item->name, new_full_name);
  }
}
...
void Simulate::LoadOnRenderThread() {
  ...
  if (true/*!this->platform_ui->IsGPUAccelerated()*/) {
...
void Simulate::Render() {
  ...
  // update state monitoring section (real-time values)
  if (this->ui1_enable && this->state_sect_id_ >= 0) {
    UpdateStateSection(this);
    // 强制 UI 刷新
    mjui_update(this->state_sect_id_, -1, &this->ui1, &this->uistate, &this->platform_ui->mjr_context());
  }
...
void Simulate::RenderLoop() {
  ...
  this->opt.flags[mjRND_SHADOW] = 0;  // 禁用阴影渲染
