...
class Simulate {
 public:
  ...
  // State monitoring data structures
  struct StateItemInfo {
    int joint_id;           // 关节 ID
    int q_index;           // 在 qpos 或 qvel 数组中的全局索引
    bool is_qvel;          // true 表示 qvel，false 表示 qpos
    std::string descriptor; // 描述符 (如 "qw", "px", "angle" 等)
    int ui_item_id;        // 在 UI 中的 item ID
  };
  std::vector<StateItemInfo> state_item_infos_;
  int state_sect_id_ = -1;  // State Section 的 ID
